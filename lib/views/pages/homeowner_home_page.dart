...
class _HomeownerHomePageState extends State<HomeownerHomePage> {
  ...
  Widget build(BuildContext context) { 
    ...
    {'key': 'All', 'name': l10n.categoryAll, 'icon': Icons.apps},
    {'key': 'Plumbing', 'name': l10n.categoryPlumbing, 'icon': Icons.plumbing_outlined},
    {'key': 'Electrical', 'name': l10n.categoryElectrical, 'icon': Icons.electrical_services_outlined},
    {'key': 'Cleaning', 'name': l10n.categoryCleaning, 'icon': Icons.cleaning_services_outlined},
    {'key': 'Air Cond', 'name': l10n.categoryAirCond, 'icon': Icons.ac_unit_outlined},
    {'key': 'Painting', 'name': l10n.categoryPainting, 'icon': Icons.format_paint_outlined},
    {'key': 'Carpentry', 'name': l10n.categoryCarpentry, 'icon': Icons.carpenter_outlined},
    {'key': 'More', 'name': l10n.categoryMore, 'icon': Icons.more_horiz},
  ];
    return RefreshIndicator( onRefresh: _loadHomepageData, child: ListView( padding: const EdgeInsets.all(16.0), children: [
        _buildWelcomeMessage(l10n), const SizedBox(height: 16), _buildSearchBar(l10n), const SizedBox(height: 20), _buildSectionHeader(l10n.specialOffers), _buildOffersCarousel(), const SizedBox(height: 24), _buildSectionHeader(l10n.categories), _buildCategoryList(_categories), const SizedBox(height: 24), _buildSectionHeader(l10n.allServices), _buildAllServicesGrid(),
      ],),);
  }

  Widget _buildWelcomeMessage(AppLocalizations l10n) {  return Text('${l10n.welcome}, $_userName!', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w600),); }
  Widget _buildSearchBar(AppLocalizations l10n) {  final bool isNearbyActive = _filterByCurrentLocationActive; final IconData locationIcon = isNearbyActive ? Icons.location_on : Icons.location_on_outlined; final Color locationIconColor = isNearbyActive ? Theme.of(context).colorScheme.error : Theme.of(context).primaryColor; String tooltipMessage = 'Tap to show services near your current location'; if (isNearbyActive) { if (_currentLocationDistrict != null && _currentLocationDistrict!.isNotEmpty) { tooltipMessage = 'Showing services in $_currentLocationDistrict, ${_currentLocationState ?? "your state"} (Tap to show all)'; } else if (_currentLocationState != null && _currentLocationState!.isNotEmpty) { tooltipMessage = 'Showing services in $_currentLocationState (Tap to show all)'; } else { tooltipMessage = 'Showing services nearby (Tap to show all)'; } } return Row( children: [ Expanded(child: Card(elevation: 2.0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(25.0)), child: Padding(padding: const EdgeInsets.symmetric(vertical: 4.0), child: TextField(controller: _searchController, decoration: InputDecoration(hintText: l10n.searchHintHomeowner, prefixIcon: const Icon(Icons.search, size: 20), border: InputBorder.none, contentPadding: const EdgeInsets.symmetric(vertical: 10.0, horizontal: 10.0), isDense: true,),),),),), const SizedBox(width: 8), Card(elevation: 2.0, shape: const CircleBorder(), child: _isFetchingCurrentLocation ? Container(padding: const EdgeInsets.all(12.0), width: 48, height: 48, child: const CircularProgressIndicator(strokeWidth: 2.0),) : IconButton(icon: Icon(locationIcon, color: locationIconColor), tooltip: tooltipMessage, onPressed: _handleNearbyFilterTap, ),), ], ); }
