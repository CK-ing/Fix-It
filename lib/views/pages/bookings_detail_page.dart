...
// *** Ensure correct Imports for Navigation ***
// TODO: Verify these paths match your project structure
...
// TODO: Import ChatPage when created
...
class _BookingDetailPageState extends State<BookingDetailPage> {
  ...
  void _showReasonDialog({ required String title, required String hintText, required String submitButtonText, required Color submitButtonColor, required Function(String reason) onSubmit, required String cancelText}) {
     _reasonController.clear(); final formKey = GlobalKey<FormState>();
     showDialog( context: context, barrierDismissible: true, // Allow dismissing by tapping outside now
        builder: (BuildContext context) {
           return AlertDialog(
              title: Text(title), // Simpler title
              titlePadding: const EdgeInsets.fromLTRB(24, 20, 24, 0), // Adjust padding
              content: Form( key: formKey, child: TextFormField( controller: _reasonController, maxLines: 3, textCapitalization: TextCapitalization.sentences, decoration: InputDecoration( hintText: hintText, border: const OutlineInputBorder(),), validator: (value) => (value == null || value.trim().isEmpty) ? 'Reason cannot be empty.' : null, ),),
              buttonPadding: const EdgeInsets.symmetric(horizontal: 8.0, vertical: 8.0),
              actionsAlignment: MainAxisAlignment.end,
              contentPadding: const EdgeInsets.fromLTRB(24, 12, 24, 12),
              // *** Standard Cancel TextButton and Submit ElevatedButton ***
              actions: <Widget>[
                 TextButton(
                    child: Text(cancelText), // Use dynamic cancel text
                    onPressed: () => Navigator.of(context).pop(),
                 ),
                 ElevatedButton(
                    style: ElevatedButton.styleFrom( backgroundColor: submitButtonColor, padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 16), textStyle: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),),
                    onPressed: () { if (formKey.currentState!.validate()) { Navigator.of(context).pop(); onSubmit(_reasonController.text.trim()); } },
